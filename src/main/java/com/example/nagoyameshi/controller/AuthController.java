...
public class AuthController {
	...
	public String signup(@ModelAttribute @Validated SignupForm signupForm, BindingResult bindingResult, RedirectAttributes redirectAttributes, HttpServletRequest httpServletRequest) {
		...
		if(bindingResult.hasErrors()) { //エラーがある時
			return "auth/signup";
		}
		...
		redirectAttributes.addFlashAttribute("successMessage", "ご入力いただいたメールアドレスに認証メールを送信しました。メールに記載されているリンクをクリックし、会員登録を完了させてください。");
		
		return "redirect:/";
	}
	
	@GetMapping("/signup/verify") //メール認証用URL
	public String verify(@RequestParam(name = "token") String token, Model model) {
		 VerificationTokens verificationToken = verificationTokenService.getVerificationToken(token);
